...
(function () {
  ...
  /* ------------------------------------------------------------------
     МОБИЛЬНАЯ ВЫСОТА ВЬЮПОРТА (--app-height)
     -------------------------------------------------------------------
     Мобильные браузеры меняют реально видимую высоту страницы на лету
     (появление/скрытие адресной строки, экранной клавиатуры), а `100vh`
     эти изменения не учитывает — из-за этого элементы, привязанные к
     низу экрана через vh, могут физически оказаться ниже видимой
     области. CSS уже даёт неплохой дефолт через 100dvh (см. :root в
     style.css), но здесь мы держим значение ещё точнее — на основе
     window.visualViewport (учитывает даже открытие экранной клавиатуры
     при фокусе на поле ввода головоломки) с фолбэком на window.innerHeight
     для браузеров без этого API. Пересчитывается при любом изменении
     размеров, чтобы игра оставалась корректной при повороте экрана и т.п.
     ------------------------------------------------------------------ */
  ...
  function showEndScreen() {
    ...
    // Единственный "видимый в эпилоге" след спасения Клары Тенн (см.
    // klaraCanBeSaved/saved_klara в data.js): она не участвует в дальнейшем
    // сюжете второй половины (её судьба сознательно оставлена открытой,
    // чтобы не переписывать заново сбалансированную механику Акта 2), но
    // получает честную, тревожную строку в КАЖДОЙ из шести концовок —
    // маленькая победа, которая не отменяет общей обречённости истории.
    if (state.flags.saved_klara) {
      el.endScreenText.textContent +=
        "\n\nКлару Тенн, которую вы спасли в ту ночь, больше никто не видел — " +
        "она исчезла с острова за день до того, как всё закончилось. Жива ли она, вы не знаете до сих пор.";
    }
